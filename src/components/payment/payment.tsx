...
const Payment = () => {
  ...
  return (
    <div className="max-w-screen-lg w-full px-6 lg:mx-auto lg:min-h-[500px] h-dvh my-10">
      ...
      <div className="text-[21px] text-[#333] font-semibold pt-[30px] pb-[10px]">Nội dung thanh toán</div>
      <div className="border border-solid border-[#7e8eaa] px-8 py-5 max-w-full">
        <p className="pb-[10px]">
          <span className="text-[18px] text-[#0e1d2f] font-medium w-24 inline-block">Phim:</span>
          <span>{infoBookingObj?.movieName}</span>
        </p>
        <p className="pb-[10px]">
          <span className="text-[18px] text-[#0e1d2f] font-medium w-24 inline-block">Ngày:</span>
          <span>{infoBookingObj?.date}</span>
        </p>
        <p className="pb-[10px]">
          <span className="text-[18px] text-[#0e1d2f] font-medium w-24 inline-block">Thời gian:</span>
          <span>{`${infoBookingObj?.timeStart}-${infoBookingObj?.timeEnd}`}</span>
        </p>
        <p className="pb-[10px]">
          <span className="text-[18px] text-[#0e1d2f] font-medium w-24 inline-block">Ghế:</span>
          <span>{infoBookingObj?.seatNames?.join()}</span>
        </p>
        <p className="pb-[10px]">
          <span className="text-[18px] text-[#0e1d2f] font-medium w-24 inline-block">Số ghế:</span>
          <span>{infoBookingObj?.countSeat}</span>
        </p>
        <p className="pb-[10px]">
          <span className="text-[18px] text-[#0e1d2f] font-medium w-24 inline-block">Tổng tiền:</span>
          <span>
